...
class account_aged_partner_balance_vw(osv.TransientModel):
    ...
    _columns = {
        ...
        'zona_id': fields.related('partner_id', 'zona_id', type='many2one', relation='glp.zonas', string='Zona', readonly=True),
        'salesperson_user_id': fields.related('partner_id', 'user_id', type='many2one', relation='res.users', string='Salesman', readonly=True),
...
class account_aged_trial_balance(osv.TransientModel):
    ...
    def compute_lines(self, cr, uid, ids, context=None):
        ...
        if wzd_brw.type in ('variation', 'distributed'):
            ...
            apb_obj = self.pool.get('account.aged.partner.balance.vw')
            apbids = apb_obj.search(cr, uid, [], context=context)
            apb_obj.unlink(cr,uid,apbids, context=context)
